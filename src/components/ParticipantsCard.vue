<template>
  <div v-if="participants && participants.length > 0"
    class="tw-w-full tw-bg-white tw-p-6 tw-border tw-border-[#E8EBEF] tw-space-y-3">
    <h4 class="tw-text-xl tw-text-[#0F1419]">Participants ({{ participants.length }})</h4>

    <p class="tw-text-base tw-text-[#333333]">
      Participants will be able to withdraw their contributions in the order of the number assigned below. You can
    ...
    <ul class="tw-space-y-6 tw-pt-4">
      <li v-for="(participant, index) in participants" :key="participant.id" class="tw-flex tw-items-center tw-gap-3">
        <p :id="`slot-${index}`" @click="assignSlot(participant)" :class="participant.new_slot ? 'bgR' : 'bgP'"
        ...
        <p class="tw-text-base tw-text-[#000000] tw-flex tw-relative">
          {{ participant.email }}
          <span class="tw-absolute  tw-ml-2 tw-flex tw-items-center -tw-right-16 z-10"
            v-if="participant.slot_request && isAjoOwner" :id="`slot-request-${index}`">

            <Button slotPos="left" icon="pi pi-arrow-right-arrow-left" :badge="participant.slot_request" label="Accept"
              size="small" class=" tw-h-6 !tw-text-white " color="danger" @click.stop="acceptSlot(participant)">
            </Button>
          </span>
        </p>
      </li>
    ...
    const slotRequests = this.participants.filter(participant => participant.slot_request != 0)
      slotRequests.forEach((slotRequest, index) => {
        steps.push({
          ...
          popover: { title: 'Change of Slot Request', 
          //showButtons: [],
